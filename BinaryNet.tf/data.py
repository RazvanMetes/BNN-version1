...
def __read_cifar(filenames, shuffle=True, cifar100=False):
  ...
  print("python main.py --model BNN_cifar10 --save BNN_cifar10 --dataset cifar10",label_bytes)
  height = 32
  width = 32
  depth = 3
...
def get_data_provider(name, training=True, mnist_input=None):
    if name == 'imagenet':
        ...
        if training:
            ...
            #return DataProvider(__read_cifar([os.path.join(data_dir, 'train.bin')]), [50000, 32,32,3], True)
        else:
            return DataProvider([os.path.join(data_dir, 'test.bin')],
                               10000, False, __read_cifar)
            #return DataProvider(__read_cifar([os.path.join(data_dir, 'test.bin')]), [50000, 32, 32, 3], False)
    elif name == 'mnist':
